...
Create ADRs when significant architectural decisions need to be documented.
Choose only the categories relevant to the request (typically 3-6), such as:
- Core platform choices (frameworks, libraries, languages)
- Data architecture (database selection, modeling approach, consistency)
- Interface architecture (API patterns, frontend architecture, state management)
- Security and reliability (authentication/authorization, quality gates, observability)
- Runtime architecture (deployment, infrastructure, cloud service selections)
- Integration patterns (messaging, caching, orchestration)
...
- Migration path from current state when this ADR replaces or changes an existing approach
