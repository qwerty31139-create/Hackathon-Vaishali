...
handoffs: 
  - label: Create PRD (/prd)
    ...
    prompt: /prd
    send: false
  - label: Review PRD for OutSystems Feasibility
    agent: devlead
    prompt: Review the PRD for OutSystems technical feasibility, completeness, and identify any missing requirements to support OutSystems implementation. Focus on simplicity-first approach.
    send: false
  - label: Break PRD into FRDs (/frd)
    agent: pm
    prompt: /frd
    send: false
  - label: Review FRD for OutSystems Completeness
    agent: devlead
    prompt: Review the FRD for OutSystems technical feasibility, completeness, and identify any missing requirements to support OutSystems implementation. Ensure minimal viable requirements are captured.
    send: false
  - label: Generate ADRs
    agent: architect
